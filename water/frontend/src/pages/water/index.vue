<template>
  <div class="water-monitor">
    <div class="content">
      ...
      <!--            &lt;!&ndash; 步骤一的特殊处理 &ndash;&gt;-->
      <!--            <template v-if="index === 0">-->
      <!--              <p>(1) 点击右侧【区块链可视化引擎】，进入合约管理/合约IDE中部署编写好的智能合约；部署时填入下方养殖场及监管局区块链账户地址</p>-->
      ...
      <!--              </div>-->
      <!--              <p>(2) 点击右侧【养殖户】身份进行登录，填写上方自动生成的密码, 进入养殖户登录界面。</p>-->
      <!--            </template>-->
      <!--            &lt;!&ndash; 其他步骤使用普通的 v-html &ndash;&gt;-->
      <!--            <template v-else>-->
      ...
      <!-- 右侧主显示区 -->
      <div class="main-view">
        <div class="title">海水环境可信水质数据监管平台</div>
        <div class="content-area">
          <!-- 未登录时显示卡片和登录组件 -->
          <div v-if="!isLoggedIn">
            <!-- 显示卡片选择 -->
            <div class="card-container" v-if="!currentLoginType">
              <div class="card" style="background-image: linear-gradient(to right, #ffffff, #fdfdfe, #fbfafc, #f8f8fb, #f6f6fa, #f1f2f9, #ecedf8, #e6e9f7, #dde1f7, #d3d9f6, #cad2f6, #c1caf5);" @click="handleCardClick('blockchain')">
                <div class="card-title">区块链可视化引擎</div>
              </div>
              <div class="card" style="background-image: linear-gradient(to right, #ffffff, #fdfdfe, #fbfafc, #f8f8fb, #f6f6fa, #f1f2f9, #ecedf8, #e6e9f7, #dde1f7, #d3d9f6, #cad2f6, #c1caf5);" @click="handleCardClick('enterprise')">
                <div class="card-title">养殖户登录</div>
              </div>
              <div class="card" style="background-image: linear-gradient(to right, #ffffff, #fdfdfe, #fbfafc, #f8f8fb, #f6f6fa, #f1f2f9, #ecedf8, #e6e9f7, #dde1f7, #d3d9f6, #cad2f6, #c1caf5);" @click="handleCardClick('monitor')">
              ...
              <div :key="18" class="enterprise-login">
                  <div class="login-container" v-show="currentLoginType === 'enterprise'">
                    <div class="login-title">
                      <h2>养殖户登录</h2>
                      <div class="login-subtitle">欢迎使用海水环境可信水质数据监管系统</div>
                    </div>
                ...
                <div class="monitor-login" style="" v-show="currentLoginType === 'monitor'">
                  <div :key="15" class="login-container">
                    <div class="login-title">
                      <h2>监管局登录</h2>
                      <div class="login-subtitle">水环境可信水质数据监管系统</div>
                    </div>
                    <t-form ref="form" :data="formData" :rules="rules" @submit="handleLoginSuccess">
                      <t-form-item name="username">
                        <t-input
...
import uploadXuke from '@/assets/images2/uploadXuke.jpg'
import bangding from '@/assets/images2/bangding.jpg'
...
export default {
    ...
    return {
      ...
      steps: [
        ...
        },
        {
          image: uploadXuke,
          visible: false,
          title: '步骤二：养殖户进行排污许可申请',
          content: `
            <p>(1)登录养殖户区块链账户后，选择【养殖户】身份登录，账号密码参考【步骤一】，进行排污许可申请</p>
            <p>(2) 点击申请许可，上传排污许可证；</p>
            <p>(3) 上传成功后查看排污许可证状态</p>
          `
        },
        {
          image: bangding,
        ...
        },
        {
          ...
          title: '步骤四：养殖户上传水质数据',
          content: `
            <p>(1) 操作物联网设备，获取水质数据；</p>
            <p>(2) 通过物联网设备与后端讲数据传输到平台。</p>
          `
        },
        {
          title: '步骤五：监管局审核排污数据并上链',
          content: `
            <p>(1) 监管局登录系统, 账号密码参考【步骤一】；</p>
            <p>(2) 查询养殖户上传的水质数据；</p>
            <p>(3) 进行数据审核上链操作。</p>
  ...
  methods: {
    ...
    //   this.currentUser = userType === 'enterprise' ? '企业用户' : '监管局用户';
    // },
    logout() {
...
.water-monitor {
  ...
  .content {
    ...
    .main-view {
      ...
      flex-direction: column;
      align-items: center;
      background-image: url('@/assets/images2/beijing4.jpg');
      overflow: hidden;
      .title {
        font-size: 24px;
        font-weight: bold;
